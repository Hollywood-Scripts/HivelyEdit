...
int stereopan_left[]  = { 128,  96,  64,  32,   0 };
int stereopan_right[] = { 128, 160, 193, 225, 255 };
...
signed char waves[WAVES_SIZE];
short waves2[WAVES_SIZE];
...
ULONG panning_left[256], panning_right[256];
...
void hvl_GenPanningTables( void )
{
  ...
  for( i=0; i<256; i++ )
  {
    panning_left[i]  = (ULONG)(sin(aa)*255.0f);
    panning_right[i] = (ULONG)(sin(ab)*255.0f);
...
void hvl_GenSawtooth( signed char *buf, ULONG len )
{
  ULONG i;
  int  val, add;
...
void hvl_GenTriangle( signed char *buf, ULONG len )
{
  ULONG i;
  int  d2, d5, d1, d4;
  int  val;
...
void hvl_GenSquare( signed char *buf )
{
  ULONG i, j;
...
#else
static __inline double clip( double x )
#endif
{
  if( x > 127.f )
    x = 127.f;
...
void hvl_GenFilterWaves( signed char *buf, signed char *lowbuf, signed char *highbuf )
{
  static const UWORD lentab[45] = { 3, 7, 0xf, 0x1f, 0x3f, 0x7f, 3, 7, 0xf, 0x1f, 0x3f, 0x7f,
  ...
  for( temp=0, freq=8.f; temp<31; temp++, freq+=3.f )
  {
	...
	low  += mid * fre;
	low   = clip( low );
	*lowbuf++  = (signed char)low;
	*highbuf++ = (signed char)high;
      }
...
void hvl_GenWhiteNoise( signed char *buf, ULONG len )
{
  ULONG ays;
...
struct hvl_tune *hvl_load_ahx( UBYTE *buf, ULONG buflen, ULONG defstereo, ULONG freq )
{
  UBYTE  *bptr;
  STRPTR nptr;
  ULONG  i, j, k, l, posn, insn, ssn, hs, trkn, trkl;
  struct hvl_tune *ht;
  ...
  ht = pure_malloc(hs); //AllocVec( hs, MEMF_ANY );
  if( !ht )
  {
    return NULL;
  }
  ...
  ht->ht_Subsongs    = (UWORD *)(&ht->ht_Instruments[(insn+1)]);
  ...
  strncpy( ht->ht_Name, (STRPTR)&buf[(buf[4]<<8)|buf[5]], 128 );
  nptr = (STRPTR)&buf[((buf[4]<<8)|buf[5])+strlen( ht->ht_Name )+1];
  ...
  for( i=0; i<ht->ht_PositionNr; i++ )
  {
    for( j=0; j<4; j++ )
    {
      ...
      ht->ht_Positions[i].pos_Transpose[j] = *(signed char *)bptr++;
    }
  }
...
struct hvl_tune *hvl_reset( UBYTE *buf, ULONG buflen, ULONG defstereo, ULONG freq)
{
  ...
  STRPTR nptr;
  UBYTE  *bptr;
  ULONG  i, j, posn, insn, ssn, chnn, hs, trkl, trkn;
  struct  hvl_plsentry *ple;
...
void hvl_process_stepfx_1( struct hvl_tune *ht, struct hvl_voice *voice, int FX, int FXParam )
{
  switch( FX )
  {
...
void hvl_process_stepfx_2( struct hvl_tune *ht, struct hvl_voice *voice, int FX, int FXParam, int *Note )
{
  switch( FX )
  {
...
void hvl_process_stepfx_3( struct hvl_tune *ht, struct hvl_voice *voice, int FX, int FXParam )
{
  int i;
...
void hvl_process_step( struct hvl_tune *ht, struct hvl_voice *voice )
{
  ...
  if( ( Instr ) && ( Instr <= ht->ht_InstrumentNr ) )
  {
    ...
    short  SquareLower, SquareUpper, d6, d3, d4;
...
void hvl_plist_command_parse( struct hvl_tune *ht, struct hvl_voice *voice, int FX, int FXParam )
{
  switch( FX )
  {
    case 0:
...
{
  static const UBYTE Offsets[] = {0x00,0x04,0x04+0x08,0x04+0x08+0x10,0x04+0x08+0x10+0x20,0x04+0x08+0x10+0x20+0x40};
...
void hvl_mixchunk( struct hvl_tune *ht, ULONG samples, signed char *buf1, signed char *buf2, int bufmod )
{
  signed char   *src[MAX_CHANNELS];
  signed char   *rsrc[MAX_CHANNELS];
  ULONG  delta[MAX_CHANNELS];
  ULONG  rdelta[MAX_CHANNELS];
  int   vol[MAX_CHANNELS];
  ULONG  pos[MAX_CHANNELS];
  ULONG  rpos[MAX_CHANNELS];
  ULONG  cnt;
  int   panl[MAX_CHANNELS];
  int   panr[MAX_CHANNELS];
//  ULONG  vu[MAX_CHANNELS];
  int   a=0, b=0, j;
  ULONG  i, chans, loops;
...
/*
int hvl_mix_findloudest( struct hvl_tune *ht, ULONG samples )
{
  signed char   *src[MAX_CHANNELS];
  signed char   *rsrc[MAX_CHANNELS];
  ULONG  delta[MAX_CHANNELS];
